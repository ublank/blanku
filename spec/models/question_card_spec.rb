...
describe QuestionCard do
  ...
  describe "when many answer cards are created that reference it" do
    it "will have many answer cards" do
      ...
      q_card = QuestionCard.create({user_id:user.id, question_text:"some text", url:"www.google.com"})
      AnswerCard.create({ user_id:user2.id, answer_text:"some text", question_card_id:q_card.id })
      AnswerCard.create({ user_id:user2.id, answer_text:"some text", question_card_id:q_card.id })
      AnswerCard.create({ user_id:user2.id, answer_text:"some text", question_card_id:q_card.id })
